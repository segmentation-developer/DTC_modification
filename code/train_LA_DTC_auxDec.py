...
from networks.vnet_sdf_addAuxDec import VNet_auxDec
...
import wandb
wandb.init(project="SSL_LA heart", config ={
        #"batch size" :16,
        #"max_iterations":5000,
        #"encoder": 'resnet-34',
        #"weights": 'imagenet',
...
parser.add_argument('--exp', type=str,
                    default='LA/DTC_unsuper_addAuxDec', help='model_name')
...
if __name__ == "__main__":
    ...
    def create_model(ema=False):
        # Network definition
        net = VNet_auxDec(n_channels=1, n_classes=num_classes-1,
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_tanh, outputs, out_random_drop, out_feature_drop, out_noise = model(volume_batch)
            ...
            loss_sdf = mse_loss(outputs_tanh[:labeled_bs, 0, ...], gt_dis)             #supervised SDF
            loss_seg = ce_loss(
                outputs[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())         #supervised seg
            loss_seg_dice = losses.dice_loss(
                outputs_soft[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)        #supervised dice seg
            dis_to_mask = torch.sigmoid(-1500*outputs_tanh)     #inverse transform(SDM -> segmentation)

            # unsupervised #################################################################################
            random_drop_loss = mse_loss(outputs[labeled_bs:], out_random_drop[labeled_bs:] )
            feature_drop_loss = mse_loss(outputs[labeled_bs:], out_feature_drop[labeled_bs:])
            noise_loss = mse_loss(outputs[labeled_bs:], out_noise[labeled_bs:])

            auxDec_loss = torch.mean(random_drop_loss + feature_drop_loss + noise_loss)

            consistency_loss = torch.mean((dis_to_mask - outputs_soft) ** 2)  # supervised, unsupervised 포함
            supervised_loss = loss_seg_dice + args.beta * loss_sdf
            consistency_weight = get_current_consistency_weight(iter_num // 150)


            loss = supervised_loss + consistency_weight * consistency_loss + (consistency_weight * 0.3) * auxDec_loss
            ###############################################################################################################
            ...
            #consistency_loss = torch.mean((dis_to_mask - outputs_soft) ** 2)    #supervised, unsupervised 포함
            #supervised_loss = loss_seg_dice + args.beta * (loss_sdf + auxDec_loss)
            #consistency_weight = get_current_consistency_weight(iter_num//150)
            ...
            wandb.log({
                ...
                "SL_seg_loss": loss_seg_dice.item(),            # segmentation loss :dice  :super
                "SL_SDM_loss": loss_sdf.item(),                     # SDM loss :super
                ...
                #"auxDec_loss" : auxDec_loss.item(),
                #"SL_dice_metrics": dc.item()
