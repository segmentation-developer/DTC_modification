...
class VNet(nn.Module):
    ...
    def decoder(self, features):
        ...
        x4_conv = self.block_four(x4)
        x3_conv = self.block_three(x3)
        x2_conv = self.block_two(x2)
        x1_conv = self.block_one(x1)

        x5_up = self.block_five_up(x5)  # 4,128,14,14,10    #upsample -> BN -> Relu
        # x5_up = x5_up + x4
        x5_up = torch.cat([x5_up, x4_conv], 1)  ##4,256,14,14,10
