...
class UnetConv3(nn.Module):
    ...
    def forward(self, n_stages, inputs):
        if n_stages == 1 :
...
class Vattention_Unet(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        self.maxpool1 = nn.Conv3d(kernel_size=(2, 2, 2))
    ...
    def forward(self, inputs):
        # Feature Extraction
        conv1 = self.conv1(1,inputs)
        maxpool1 = self.maxpool1(conv1)

        conv2 = self.conv2(2,maxpool1)
        maxpool2 = self.maxpool2(conv2)

        conv3 = self.conv3(3,maxpool2)
        maxpool3 = self.maxpool3(conv3)

        conv4 = self.conv4(3,maxpool3)
        maxpool4 = self.maxpool4(conv4)

        # Gating Signal Generation
        center = self.center(3,maxpool4)
