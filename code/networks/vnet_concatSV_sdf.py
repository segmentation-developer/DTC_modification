...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        self.block_seven = convBlock(3, n_filters * 8 + n_filters * 4, n_filters * 4, normalization=normalization)
        self.block_seven_up = UpsamplingDeconvBlock(n_filters * 4, n_filters * 2, normalization=normalization)

        self.block_eight = convBlock(2, n_filters * 4 + n_filters * 2, n_filters * 2, normalization=normalization)
        self.block_eight_up = UpsamplingDeconvBlock(n_filters * 2, n_filters, normalization=normalization)

        self.block_nine = convBlock(2, n_filters * 2 + n_filters, n_filters, normalization=normalization)
    ...
    def decoder(self, features):
        ...
        #x6_up = x6_up + x3
        x4_up = self.block_six_up(x4)       #4,128,14,14,10 ->4,64,28,28,20
        x6_up = torch.cat([x6_up , x3, x4_up], 1)      #4, 128+64 , 28,28,20

        x7 = self.block_seven(x6_up)            #4, 128+64 , 28,28,20 -> 4, 64 , 28,28,20
        x7_up = self.block_seven_up(x7)     # 4,32,56,56,40
        #x7_up = x7_up + x2
        x3_up = self.block_seven_up(x3)         # 4,32,56,56,40
        x7_up = torch.cat([x7_up, x2, x3_up], 1)           #4, 64+32 ,56,56,40

        x8 = self.block_eight(x7_up)            #4,32,56,56,40
        x8_up = self.block_eight_up(x8)         #4,16,112,,112,80
        #x8_up = x8_up + x1
        x2_up = self.block_eight_up(x2)
        x8_up = torch.cat([x8_up, x1, x2_up], 1)         #4, 32+16 ,112,,112,80
