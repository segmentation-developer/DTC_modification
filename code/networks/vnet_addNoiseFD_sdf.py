...
class VNet(nn.Module):
    ...
    def decoder(self, features):
        ...
        noise_seg = self.feature_based_noise(x9)        #x_noise = x.mul(noise_vector) + x
        noise_seg = self.out_conv3(noise_seg)

        fd_seg = self.feature_dropout(x9)
        fd_seg = self.out_conv4(fd_seg)

        return out_tanh, out_seg ,noise_seg , fd_seg
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        out_tanh, out_seg ,noise_seg , fd_seg= self.decoder(features)
        if turnoff_drop:
            self.has_dropout = has_dropout
        return out_tanh, out_seg ,noise_seg , fd_seg
