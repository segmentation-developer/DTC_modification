...
from networks.vnet_DenseModi_decPointwise_concat import VNet
...
parser.add_argument('--exp', type=str,
                    default='LA/SSL_DTC_DenseModi_decPointwise_conandUnet', help='model_name')
...
if __name__ == "__main__":
    ...
    model1 = create_model()
    model2 = unet_3D(in_channels=1, n_classes=num_classes-1).cuda()
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # segmentation loss
            loss_seg1_ce = ce_loss(
                outputs1[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())
            loss_seg1_dice = losses.dice_loss(
                outputs_soft1[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)

            loss1 = loss_seg1_ce + loss_seg1_dice

            loss_seg2_ce = ce_loss(
                outputs2[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())
            loss_seg2_dice = losses.dice_loss(
                outputs_soft2[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)

            loss2 = loss_seg2_ce + loss_seg2_dice

            consistency_loss = torch.mean((outputs_soft1 - outputs_soft2) ** 2)
            ...
            writer.add_scalar('loss/loss_seg1', loss1, iter_num)
            writer.add_scalar('loss/loss_seg2', loss2, iter_num)
            ...
            wandb.log({
                ...
                "SL_seg1_Loss": loss1.item(),  # segmentation loss :dice  :super
                "SL_seg2_Loss": loss2.item(),
