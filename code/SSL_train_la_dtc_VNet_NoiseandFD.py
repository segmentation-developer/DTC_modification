...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_tanh, outputs, outputs_noise , outputs_fd = model(volume_batch)
            outputs_soft = torch.sigmoid(outputs)
            outputs_noise_soft = torch.sigmoid(outputs_noise)
            outputs_fd_soft = torch.sigmoid(outputs_fd)
            ...
            #loss_BD = losses_2.boundary_loss(outputs_soft[:labeled_bs, 0, ...], gt_dis)
            ...
            # noise segmentation loss
            loss_noise_ce = ce_loss(
                outputs_noise[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())
            loss_noise_dice = losses.dice_loss(
                outputs_noise_soft[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)

            loss_fd_ce = ce_loss(
                outputs_fd[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())
            loss_fd_dice = losses.dice_loss(
                outputs_fd_soft[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)
            ...
            supervised_loss = loss_seg_dice + loss_seg + args.beta * loss_sdf + (0.5) * (loss_noise_ce +loss_noise_dice +loss_fd_ce +loss_fd_dice)
            ...
            wandb.log({
                ...
                "SL_seg_Loss": loss_seg_dice.item() + loss_seg.item(),  # segmentation loss :dice  :super
                "SL_SDM_loss": loss_sdf.item(),
                "SL_noise_seg_Loss": loss_noise_dice.item() + loss_noise_ce.item(),
                "SL_fd_seg_Loss": loss_fd_dice.item() + loss_fd_ce.item(),
