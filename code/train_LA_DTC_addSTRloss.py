...
parser.add_argument('--exp', type=str,
                    default='LA/DTC_STR', help='model_name')
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                gt_dis = torch.from_numpy(gt_dis).float().cuda()        #gt_dis :[2,112,112,80]
            ...
            seg_to_dis = compute_sdf(outputs_soft.cpu().detach().numpy(), outputs_tanh[ : , 0, ...].shape)
            seg_to_dis = torch.from_numpy(seg_to_dis).float().cuda()

            consistency_loss = torch.mean((dis_to_mask - outputs_soft) ** 2)    #supervised, unsupervised 포함
            STR_loss = torch.mean((seg_to_dis - outputs_tanh) ** 2)          #supervised
            supervised_loss = loss_seg_dice + args.beta * loss_sdf
            consistency_weight = get_current_consistency_weight(iter_num//150)

            loss = supervised_loss + (consistency_weight/2) * (consistency_loss + STR_loss)
            ...
            wandb.log({
                ...
                "SL_seg_loss": loss_seg_dice.item(),
