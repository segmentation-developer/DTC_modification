...
parser.add_argument('--root_path', type=str,
                    default='/home/psh/Downloads/data/LA_dataset/2018LA_Seg_TrainingSet', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='LA/SL_attentionNet_diceLoss1', help='model_name')
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #    ).numpy(), outputs[:, 0, ...].shape)
            #    gt_dis = torch.from_numpy(gt_dis).float().cuda()
            #loss_sdf = mse_loss(outputs_tanh[:, 0, ...], gt_dis)
            #loss_seg = ce_loss(
            #    outputs[:, 0, ...], label_batch[:].float())
            loss_seg_dice = losses.dice_loss1(                       # why dice_loss instead of dice_loss1 ?
                outputs_soft[:, 0, :, :, :], label_batch[:] == 1)
            ...
            wandb.log({
                ...
                "SL_seg_loss1": loss_seg_dice.item(),  # segmentation loss :dice  :super
                #"SL_SDM_loss": loss_sdf.item(),
                #"supervised_loss": supervised_loss.item(),
                #"TSA_supervised_loss": tsa_supervised_loss.item(),
            ...
            logging.info(
                'iteration %d : loss : %f, loss_dice1: %f' %
